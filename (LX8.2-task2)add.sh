...
if [ "$answer_code" -eq "$cod" ]; then
echo "$times from $host according to the protocol $protocol request was executed type $type to get the resourse located at the link $host_link. The response code to the request from server: $answer_code. $bite bytes passed. The request was transmitted from the site $link_site. The client used program $program, client\`s OS: $os"
fi
done
